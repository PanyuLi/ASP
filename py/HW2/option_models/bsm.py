...
class BsmModel:
    ...
    def delta(self, strike, spot, texp, cp=1):
        ...
        if (texp < 0 or self.vol * np.sqrt(texp) < 1e-10):
            return disc_fac * np.fmax(cp * (forward - strike), 0)

        vol_std = self.vol * np.sqrt(texp)
        d1 = np.log(forward / strike) / vol_std + 0.5 * vol_std

        delta = cp * disc_fac * ss.norm.cdf(cp * d1)
        return delta
    ...
    def vega(self, strike, spot, vol, texp, cp=1):
        ...
        vega = disc_fac * forward * ss.norm.pdf(d1) * np.sqrt(texp)
        return vega
    ...
    def gamma(self, strike, spot, vol, texp, cp=1):
        ...
        gamma = disc_fac * ss.norm.pdf(d1) / (forward * vol_std)
        return gamma
