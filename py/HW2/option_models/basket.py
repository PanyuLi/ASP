...
def basket_price_mc(
    ...
    cov_m = vol * cor_m * vol[:, None]   # variance
    chol_m = np.linalg.cholesky(cov_m)   # sigma
    ...
    if (bsm):
        ...
        '''
        vol_m = np.diagonal(cov_m)[:, None]
        prices = forward[:, None] * np.exp(-0.5 * vol_m * texp + np.sqrt(texp) * chol_m @ znorm_m)
...
def basket_price_norm_analytic(
    ...
    cov_n = vol * cor_m * vol[:, None]
    vol_n = np.sqrt(weights @ cov_n @ weights)
    price = disc_fac * normal_formula(strike, forward_basket, vol_n, texp, intr=0, divr=0, cp=cp)
